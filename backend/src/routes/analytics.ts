...
router.get('/revenue/overview', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    // Calculate total revenue by combining ride and e-commerce revenue (same currency)
...
// Get detailed breakdown of ride revenue
router.get('/revenue/ride-revenue-details', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
...
// Get detailed breakdown of e-commerce revenue
router.get('/revenue/ecommerce-revenue-details', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30', page = '1', limit = '10', currency = 'GMD' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));
...
// Get available currencies from both rides and orders
router.get('/revenue/available-currencies', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const [rideCurrencies, orderCurrencies] = await Promise.all([
      // Get currencies from ride requests
...
// Get external transaction analytics overview
router.get('/revenue/external-transactions-overview', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30', currency = 'GMD' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));

    // Get external transactions grouped by transaction type and app service
    ...
    // Group service fees by service type
...
// Get detailed breakdown of external transactions
router.get('/revenue/external-transactions-details', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30', page = '1', limit = '10', currency = 'GMD' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));
...
// Get detailed breakdown of total revenue
router.get('/revenue/total-revenue-details', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30', page = '1', limit = '10', currency = 'GMD' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));
    ...
    // Get all ride transactions (we'll handle pagination after combining)
    ...
    // Get all e-commerce transactions (we'll handle pagination after combining)
...
// Ride Analytics - Get drivers that do not have a ride records or complete a ride for a period
router.get('/revenue/ride/inactive-drivers', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
...
// Ride Analytics - Get all drivers with accepted, inprogress, or completed status with no payment status of "paid"
router.get('/revenue/ride/unpaid-rides', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
...
// Ride Analytics - Get drivers that have paid rides but settlement status is not "SETTLED"
router.get('/revenue/ride/unsettled-drivers', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30', currency = 'GMD' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));
    ...
    const driversWithPaidRides = await prisma.ride.groupBy({
      ...
      where: {
        ...
        paymentStatus: 'PAID',
        settlementStatus: {
          not: 'SETTLED', // Get rides that are not settled
        },
        createdAt: { gte: startDate },
        rideRequest: {
          currency: currency as string,
    ...
    const unsettledDriversWithDetails = await Promise.all(
      driversWithPaidRides.map(async (driver) => {
...
// Fault cases - Ride - Get driver that cover high distance and the trip amount is not paid
router.get('/revenue/ride/high-distance-unpaid', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
...
// Fault cases - Ride - Get drivers the give a ride to one customer more than once time a day
router.get('/revenue/ride/repeat-customers', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const repeatCustomers = Object.values(driverCustomerPairs).filter((pair: any) => pair.rides.length > 1);

    res.json({
      date: targetDate.toISOString().split('T')[0],
      totalRides: ridesForDate.length,
      repeatCustomerPairs: repeatCustomers.length,
      repeatCustomers: repeatCustomers.map((pair: any) => ({
        ...
        totalFare: pair.rides.reduce((sum: number, ride: any) => {
...
// Fault cases - Ride - Get rides that are accepted but have no ride_token record or have unused tokens
router.get('/revenue/ride/no-token-rides', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { currency = 'GMD' } = req.query;
    const ridesWithoutToken = await prisma.ride.findMany({
      where: {
        ...
        OR: [
          {
            // Rides with no ride token record
            rideToken: null,
          },
          {
            // Rides with ride token but isUsed = false
            rideToken: {
              isUsed: false,
            },
          },
...
// Fault cases - e-commerce - All orders that are not canceled and payment status is pending
router.get('/revenue/ecommerce/unpaid-orders', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { currency = 'GMD' } = req.query;
...
// Fault cases - e-commerce - Sellers that has no orders on the products
router.get('/revenue/ecommerce/sellers-no-orders', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));
...
// Fault cases - e-commerce - Get all sellers with paid orders but no settlement request or incomplete settlements
router.get('/revenue/ecommerce/sellers-unsettled', authenticate, requirePermission('ANALYTICS', 'VIEW'), async (req: AuthenticatedRequest, res) => {
  try {
    const { period = '30', currency = 'GMD' } = req.query;
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - Number(period));
    ...
    // Group settlements by seller and get their status
    ...
    const unsettledSellers = sellersWithPaidOrders.map(seller => {
      ...
      // If has settlements but none are completed
      if (!hasCompletedSettlement) {
        const currentStatus = sellerSettlementStatuses[sellerSettlementStatuses.length - 1]; // Get latest status
        return {
          ...seller,
      ...
      // If has completed settlement, exclude from results
      return null;
    }).filter(seller => seller !== null);

    const unsettledSellersWithDetails = await Promise.all(
      unsettledSellers.map(async (seller) => {
