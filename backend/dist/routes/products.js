...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        const { page = 1, limit = 10, search = '', status = 'all', condition = 'all', categoryId = 'all', isFeatured = 'all', dateFrom = '', dateTo = '', } = req.query;
...
router.get('/export', auth_1.authenticate, async (req, res) => {
    try {
        const { search = '', status = 'all', condition = 'all', categoryId = 'all', isFeatured = 'all', dateFrom = '', dateTo = '', } = req.query;
        const where = {};
        if (search) {
            where.OR = [
                { title: { contains: search, mode: 'insensitive' } },
