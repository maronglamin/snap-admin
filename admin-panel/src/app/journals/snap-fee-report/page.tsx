...
import { exportSnapFeeTransactionsToCSV, SnapFeeTransactionForExport } from '@/utils/csvExport';

interface SnapFeeTransaction {
  id: string;
  orderId: string;
  customerId: string;
  sellerId: string;
...
export default function SnapFeeReportPage() {
  const [transactions, setTransactions] = useState<SnapFeeTransaction[]>([]);
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<SnapFeeTransaction | null>(null);
  ...
  const loadTransactions = async () => {
    try {
      ...
      const response = await journalsApi.getSnapFees(params);
  ...
  const handleViewDetail = async (transaction: SnapFeeTransaction) => {
    try {
      const response = await journalsApi.getSnapFeeById(transaction.id);
      if (response.success) {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await journalsApi.exportSnapFees(params);
      
      if (response.success) {
        // Transform the data for CSV export
        const exportData: SnapFeeTransactionForExport[] = response.data.map((transaction: SnapFeeTransaction) => ({
          id: transaction.id,
        ...
        exportSnapFeeTransactionsToCSV(exportData, filename);
    ...
    return (
      <div className="p-6 flex items-center justify-center min-h-[400px]">
        <div className="flex items-center gap-2">
          ...
          <span>Loading Snap fee transactions...</span>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Snap Fee Report</p>
            </div>
          </div>
        </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center space-x-2">
                <DollarSign className="h-5 w-5" />
                <span>Snap Fee Report</span>
              </CardTitle>
              <CardDescription>
                View and manage Snap service fee transactions
              </CardDescription>
            </div>
            <Button 
        ...
        <CardContent>
          ...
          {exportSuccess && (
            <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm text-green-800">Snap fees report exported successfully!</p>
                </div>
              </div>
            </div>
          )}
          ...
          {transactions.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">No Snap fee transactions found</h3>
              <p className="mt-2 text-sm text-gray-500">
                No Snap service fee transactions match your current filters.
              </p>
            </div>
          )}
        </CardContent>
