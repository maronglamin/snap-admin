...
import { exportAuditTransactionsToCSV, AuditTransactionForExport } from '@/utils/csvExport';
...
export default function TransactionLogPage() {
  ...
  const loadTransactions = async () => {
    try {
      ...
      const response = await journalsApi.getAuditTransactions(params);
  ...
  const handleViewDetail = async (transaction: AuditTransaction) => {
    try {
      const response = await journalsApi.getAuditTransactionById(transaction.id);
      if (response.success) {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await journalsApi.exportAuditTransactions(params);
      
      if (response.success) {
        // Transform the data for CSV export
        const exportData: AuditTransactionForExport[] = response.data.map((transaction: AuditTransaction) => ({
          id: transaction.id,
        ...
        exportAuditTransactionsToCSV(exportData, filename);
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">Transaction Log</p>
            </div>
          </div>
        </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                View and log all transaction types (Original, Fee, Service Fee)
              </CardDescription>
            </div>
            <Button 
        ...
        <CardContent>
          ...
          {exportSuccess && (
            <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm text-green-800">Transaction log exported successfully!</p>
                </div>
              </div>
            </div>
          )}
          ...
          {transactions.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">No transaction logs found</h3>
              <p className="mt-2 text-sm text-gray-500">
                No transaction logs match your current filters.
              </p>
            </div>
          )}
        </CardContent>
