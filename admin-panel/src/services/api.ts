...
export const authenticationApi = {
  getDeviceNotifications: async (params?: any) => {
    const queryString = new URLSearchParams(params || {}).toString();
    const response = await fetch(`${API_BASE_URL}/authentication/device-authentication?${queryString}`, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${getAuthToken()}`,
      },
  ...
  getDeviceNotificationById: async (id: string) => {
    const response = await fetch(`${API_BASE_URL}/authentication/device-authentication/${id}`, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${getAuthToken()}`,
      },
